...
class test extends model
{
    ...
    {
        $db = $this->db('article_content');
        $data['aid'] = '186';
        $data['content'] = '186' . mt_rand(10000000, 99999999);
    ...
    public function FindData()
    {
        $db = $this->db();
        $where = 'aid > 0';
        $result = $db->table('article_content')->where($where)->find();
        return $result;
    }
    public function SelectData()
    {
        ...
        $where['aid >'] = 0;
        $result = $db->table('article_content')->where($where)->limit($limit)->select();
        return $result;
    }
    public function PageSelect0()
    {
        ...
        $result['data'] = $db->table('article_content')->page($page)->select();
    ...
    public function CacheSelect()
    {
        ...
        $num = intval($_GET['num'] ?? 0) ?: 10;
        $page = ['num' => $num, 'p' => $p, 'return' => true];
        $result['data'] = $db->table('article_content')->page($page)->cache(60)->select();
        $result['page'] = $db->getPage();
        return $result;
    }
}
